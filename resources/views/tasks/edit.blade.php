    ...
    <div class="col-lg-7">
        <div class="mb-4">
            <h1 class="h3 fw-semibold mb-1">
                <i class="fas fa-edit me-2" style="color: var(--primary);"></i>Edit Grocery Item
            </h1>
            <p class="text-muted">Update your grocery item information</p>
        </div>
            ...
            <div class="card-body p-4">
                <form method="POST" action="{{ route('tasks.update', $task) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="title" name="title" class="form-control form-control-lg @error('title') is-invalid @enderror" value="{{ old('title', $task->title) }}" required placeholder="e.g., Tomatoes, Milk, Bread">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" id="quantity" name="quantity" class="form-control form-control-lg @error('quantity') is-invalid @enderror" value="{{ old('quantity', $task->quantity) }}" min="1" required placeholder="Enter quantity">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select id="unit" name="unit" class="form-select form-control-lg @error('unit') is-invalid @enderror" required>
                                    <option value="pieces" {{ old('unit', $task->unit) === 'pieces' ? 'selected' : '' }}>Pieces</option>
                                    <option value="kg" {{ old('unit', $task->unit) === 'kg' ? 'selected' : '' }}>Kilograms (kg)</option>
                                    <option value="g" {{ old('unit', $task->unit) === 'g' ? 'selected' : '' }}>Grams (g)</option>
                                    <option value="liters" {{ old('unit', $task->unit) === 'liters' ? 'selected' : '' }}>Liters (L)</option>
                                    <option value="ml" {{ old('unit', $task->unit) === 'ml' ? 'selected' : '' }}>Milliliters (ml)</option>
                                    <option value="boxes" {{ old('unit', $task->unit) === 'boxes' ? 'selected' : '' }}>Boxes</option>
                                    <option value="packs" {{ old('unit', $task->unit) === 'packs' ? 'selected' : '' }}>Packs</option>
                                    <option value="bottles" {{ old('unit', $task->unit) === 'bottles' ? 'selected' : '' }}>Bottles</option>
                                </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="notes" name="notes" rows="4" class="form-control form-control-lg @error('notes') is-invalid @enderror" placeholder="Add any notes or preferences...">{{ old('notes', $task->notes) }}</textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="btn-group w-100" role="group">
                            <input type="radio" class="btn-check" name="status" id="status_pending" value="pending" {{ old('status', $task->status) === 'pending' ? 'checked' : '' }} required>
                            ...
                            <input type="radio" class="btn-check" name="status" id="status_bought" value="bought" {{ old('status', $task->status) === 'bought' ? 'checked' : '' }} required>
